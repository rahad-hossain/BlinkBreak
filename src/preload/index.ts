...
const electronAPI: ElectronAPI = {
  ...
  // Timer
  startTimer: (interval, duration, mode) =>
    ipcRenderer.send("start-timer", { interval, duration, mode }),
  ...
  recordBreakTaken: (duration) =>
    ipcRenderer.invoke("record-break-taken", duration),
  recordBreakSkipped: (duration) =>
    ipcRenderer.invoke("record-break-skipped", duration),

  // Blue Light Filter
  enableBlueLightFilter: (intensity) =>
    ipcRenderer.send("enable-blue-light-filter", intensity),
  disableBlueLightFilter: () => ipcRenderer.send("disable-blue-light-filter"),
  updateBlueLightIntensity: (intensity) =>
    ipcRenderer.send("update-blue-light-intensity", intensity),

  // Color Mask
  enableColorMask: (intensity, red, green, blue) =>
    ipcRenderer.send("enable-color-mask", { intensity, red, green, blue }),
  disableColorMask: () => ipcRenderer.send("disable-color-mask"),
  updateColorMask: (intensity, red, green, blue) =>
    ipcRenderer.send("update-color-mask", { intensity, red, green, blue }),

  // Monitor Controls
  setMonitorBrightness: (value) =>
    ipcRenderer.invoke("set-monitor-brightness", value),
  setMonitorContrast: (value) =>
    ipcRenderer.invoke("set-monitor-contrast", value),
  ...
  setSoundConfig: (config) => ipcRenderer.send("set-sound-config", config),

  // Smart Mode
  setSmartMode: (enabled) => ipcRenderer.send("set-smart-mode", enabled),
  setManualMeeting: (inMeeting) =>
    ipcRenderer.send("set-manual-meeting", inMeeting),
  ...
  setSmartModeWhitelist: (list) =>
    ipcRenderer.send("set-smart-mode-whitelist", list),
  ...
  setBlockedDomains: (domains) =>
    ipcRenderer.invoke("set-blocked-domains", domains),
